...
The application's `agenda_loader.py` only reads bullets that appear under
the heading below. You may rewrite the prose around it freely; do not
rename the heading without also updating the loader.
...
- Demo 4 - EOB document analysis: Drag-and-drop an EOB PDF; the app extracts member, provider, charges, and explains each line in plain English.
- Demo 5 - Evaluation dashboard: Scorecards for groundedness, harm, and a PHI-leak gauge. Compliance walks the audit view.
- Roadmap and next steps: Co-edit the 30/60/90 plan with the room. Capture commitments and named owners. No slides — work the whiteboard.
